...
**A. 内容/玩法（用户视角最敏感）**
- [x] **结论/理由位置去模板化（2026-09-19）**：重排 ch02/ch03 共 6 关中英正文，结论分布到第 3/4/5 个可点位置，可信反对意见与营销主张插入正确节点之间；ch03 改为先找理由、再归纳结论，目标栏动态展示实际顺序；未揭示节点统一样式，不再用颜色泄露类型/正误。质量门禁自动要求至少 3 种结论位置、D2/D3 干扰节点前置或交错、scan 任务同时存在两种顺序。
- [x] **逻辑法庭多卡质询与锐度实装（2026-09-19）**：ch06–09 全部 12 关从“一卡对应一处、锐度仅展示”改为破绽抗辩值机制；每处破绽必须由 2–3 张核心/补强问题累计击穿，卡片锐度直接造成实际伤害且有效使用后耗尽，错误投放不消耗。新增 9 张中英双语补强卡并重写 D1 的弱干扰题，问题库按关卡稳定打散；UI 显示每处剩余抗辩值与卡片耗尽状态。内容门禁强制每处至少两张卡、单卡伤害低于抗辩值、组合总伤害足够；Playwright 覆盖 30+10 击穿 40 的真实流程。
- [x] **数据拆弹热点语义化（2026-09-19）**：修复 ch10 把纵轴问题错误挂在柱顶的模型缺陷；`suspectSpots` 从单一 `barIndex` 升级为 `axis / bar / comparison` 三种目标。截断纵轴直接点击轴刻度并整图恢复真实基线，遗漏数据点击对应柱，相对值/过度诊断点击两组数据间的“比较”热点；D3 使用诚实的 0 基线。Zod 与质量测试强制截断轴必须存在 axis 真陷阱，并覆盖三种热点类型。
- [x] **天平校准台答案泄露修复（2026-09-19）**：移除滑块当前数值、热度/接近度进度条、偏左偏右提示与命中波纹；松手只记录一次中性尝试，提交无需先命中，未命中基础分为 0（最终分遵循全局最低 1 分规则）。复盘不再展示 `idealRange`、`idealPoint`、单次精度或方向，避免玩家靠 UI 反馈反向搜索答案区间。
...
- [x] **心智驯兽场选项位置去模式化（2026-09-19）**：ch01/ch13 的正确项不再固定在中间卡，三种位置均有覆盖；新增质量门禁要求每个驯兽关至少出现首位和末位正确项，避免玩家靠卡片位置猜答案。
...
- [x] **结算每步判定复盘（2026-09-09）**：5 引擎统一 `StepReviewItem` 判别联合与折叠复盘面板；按引擎展示目标、命中、近失误、偏题和正确回应。
- [ ] 游戏音效/震动反馈（目前纯视觉）
...
- [ ] 移动端适配检查（目前以桌面布局为主）
- [x] 自动化质量门禁（2026-09-19 更新）：独立锚点校验 + 20 项 Vitest（含 7 项灰度质量契约）+ 首访引导、透视镜反位置泄题、思考动作知识弹窗、GitHub 外链、短视口结算弹窗与 5 引擎共 11 项 Playwright 生产烟测；CI 对 push/PR 执行 lint/test/build/Pages 路径 preview
- [x] 关卡总结弹窗适配短视口：正文/复盘在弹窗内滚动、操作区固定、背景滚动锁定，并提供始终可见的关闭按钮与 Esc 退出
- [x] 首页 GitHub 入口：顶栏轻量入口 + 页尾“查看源码 · 贡献关卡”，中英双语并使用安全的新标签页跳转
- [x] “5 个思考动作”知识弹窗：每个动作含适用场景、3 个核心追问、微型示例、对应章节与练习入口；支持中英双语、短视口滚动和 Esc 关闭
