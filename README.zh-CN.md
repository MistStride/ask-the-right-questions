...
一本《学会提问》（原书第 12 版）的交互式批判性思维训练游戏集：把全书 13 章提炼成 **5 套可复用的思维引擎**。它不要求玩家背诵谬误名称，而是反复练习一件更困难的事：一条证据究竟值得我们相信到什么程度？
...
- **60 秒获得第一次认知反转**：新用户先判断一项真实感的职场研究，再看完整课程；第一分钟就体验“信息相关，却仍不足以支持结论”。
- **不是刷题，是执行思维动作**：5 套游戏引擎覆盖 13 章，每套对应一组原书思维动作（透视 / 质询 / 校准 / 拆弹 / 驯服）
- **不是非黑即白的知识问答**：选择分为命中、近失误和偏题；近失误会解释它为什么诱人，以及还没有检验哪一个核心断言。
- **内容与引擎完全分离**：关卡 = 逻辑 JSON（结构判定）+ i18n 文本包（EN/ZH），贡献者不碰任何引擎代码
- **本地进度 + 六维思维雷达**：通关点亮「结构识别力 / 证据鉴别力 / 假设挖掘力 / 谬误免疫力 / 数据免疫力 / 情绪自控力」
- **把内容标准写进代码**：Zod、语义锚点和跨引擎“灰度质量”审计会在每次生产构建前自动运行。

## 为什么做这个项目

流畅的答案越来越便宜，可靠的判断却没有。新闻、图表、销售话术、职场方案和 AI 输出都可能引用真实事实，同时推出一个过强的结论。《学会提问》提供了一套可以重复使用的方法：找到结论，检查理由与信源，寻找替代原因，再追问遗漏了什么。

真正困难的不是读懂这份清单，而是在具体情境里用出来。这个项目把每一种方法变成交互，让玩家先作出判断，再理解“一个有用的问题”和“一个足以改变结论的问题”之间的差距。
...
npm run audit:content # 单独运行“灰度题目”质量门禁
npm run build      # 校验后生产构建 → docs/（Pages 用）
npm run test:e2e   # 构建 + 首访引导与五引擎 Playwright 烟测
...
非黑即白题目的编辑标准见 **[design/CONTENT-QUALITY-RUBRIC.md](design/CONTENT-QUALITY-RUBRIC.md)**；各引擎设计文档也位于 `design/`（如 `design/ENGINE-B-DESIGN.md`）。
