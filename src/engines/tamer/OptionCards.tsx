...
interface Props {
  ...
  lastNearMissKey: string | null
  disabled: boolean
  onSelect: (key: string) => void
}

export default function OptionCards({ options, lastWrongKey, lastNearMissKey, disabled, onSelect }: Props) {
  return (
    <div className="grid gap-2.5">
      {options.map((opt) => {
        const wasWrong = lastWrongKey === opt.key
        const wasNear = lastNearMissKey === opt.key
        return (
          <button
            key={opt.key}
            type="button"
